...
> 共 173 篇论文 | 142 篇完整 + 2 篇精简中英双语 | 2026-06-03
...
在混合专家模型中，推测解码的实用性受限于草稿模型在多个专家间的高效分布。Cascade提出了一种效用驱动的推测解码方法，通过分析每个专家的推测质量来动态决定哪些专家参与草稿生成。系统根据历史推测接受率评估每个专家的效用，优先选择高价值专家生成草稿token，避免在低效用专家上浪费计算。在多种MoE模型上的评估表明Cascade在保持生成质量的同时将推测解码的计算开销显著降低。
...
生产级LLM部署缺乏系统方法来评估一致性风险。DriftBench引入了一个统一的漂移监控框架，结合在线检测和离线分析，为生产级LLM部署提供全面的漂移管理能力。它通过持续监控硬件指标、软件版本和数据特征，构建漂移概率模型，在关键性能指标退化之前发出预警。
...
在大规模分布式系统上优化LLM训练和服务是一个复杂的多目标问题。PROMPTS提出了一个通过多智能体规划来优化性能的框架，其中多个专门化智能体协作分析系统瓶颈并提出优化建议。该框架整合了训练和服务阶段的性能数据，通过智能体间的迭代规划持续改进系统配置，在吞吐量、延迟和资源利用率方面实现了全面的性能提升。
...
AI PC为消费级硬件运行LLM提供了新可能。SD-HC提出异构功能流水线技术用于推测性LLM解码，将推测解码的不同阶段分配到CPU、集成GPU和NPU上并行执行，充分利用异构处理器的计算能力，在消费级硬件上实现显著的推理加速。
...
AI应用越来越依赖长上下文推理，LLM在请求间消耗大量上下文token。ContextPilot观察到多个连续或重叠请求经常共享上下文前缀，通过智能缓存和复用这些共享前缀的KV缓存计算结果大幅降低重复计算开销。不同于LRU缓存，ContextPilot使用上下文语义相似性来预测可能复用的前缀并通过主动预热减少缓存未命中。
...
大规模推理模型因其长推理链产生高昂计算成本。本文提出利用稀疏自推测解码加速推理的方法，在自推测框架中引入稀疏性跳过不重要的草稿token验证步骤，在不影响输出质量的情况下进一步降低推理延迟，特别适用于长推理链大模型。
...
卫星星座中的ML推理面临能源和带宽约束。Equinox提出去中心化硬件感知卫星智能调度系统，将ML推理任务调度转化为去中心化优化问题，每颗卫星根据本地观测和邻居信息自主决定任务执行顺序，实现星座级协作和负载均衡。
...
分页KV缓存系统的LRU驱逐可能过早驱逐重要token。OPKV提出高吞吐量插件驱动框架用于可召回稀疏性管理，提供灵活插件接口让用户自定义缓存驱逐和召回策略，通过智能保留重要token的KV缓存并召回被驱逐的关键信息。
...
语义职位搜索需对大量职位描述和候选人资料进行高效语义匹配。本文研究将LLM服务系统扩展到语义职位搜索场景的挑战和解决方案，通过优化嵌入匹配和检索管道实现大规模职位数据和实时查询条件下的低延迟响应。
...
非结构化稀疏性在LLM推理中展现巨大加速潜力但实际部署面临效率挑战。本文提出实用的非结构化稀疏方法，通过细粒度权重剪枝和专门的稀疏计算内核，在保持模型质量的前提下大幅减少计算量实现显著推理加速。
...
Transformer分布式推理中跨节点传输KV缓存产生大量通信开销。本文提出通过向量量化上下文实现通信高效分布式推理的方法，将KV缓存向量量化为紧凑表示后进行跨节点传输，显著降低通信开销同时保持输出质量。
...
混合部署不同大小和架构的LLM越来越普遍。HiServe是面向混合LLM的前缀缓存服务系统，针对多模型混合部署场景智能管理共享前缀缓存，在保持各模型服务质量的同时最大化缓存利用率，降低多模型服务的总体基础设施成本。
...
多模态LLM推理涉及编码器、预填充和解码等多个阶段，各阶段计算特征差异显著。TriInfer提出混合EPD分离架构，将各阶段灵活分配给不同硬件资源来最大化利用率，在多模态工作负载上实现显著吞吐量提升和延迟降低。
...
不同查询对LLM推理深度需求差异很大。HELIOS提出自适应模型和早退选择方法，根据查询难度动态决定使用完整模型还是早期层输出，在每个潜在退出点评估置信度，简单查询提前退出以减少开销，复杂查询使用完整模型保证质量。
...
MoE模型中专家负载不均衡导致计算资源浪费。BLAZE提出偏置驱动的负载感知零开销专家路由方法，利用专家选择中已有的内在偏置优化路由决策，在不引入额外计算的情况下实现更均衡的专家负载分布。
...
分布式训练系统通常采用固定并行策略难以适应多变条件。Flexo是用户可控的分布式训练系统，提供直观接口让用户自定义并行策略、通信模式和资源分配，系统自动处理底层优化实现灵活性和高性能兼顾。
...
智能体AI需要高效记忆管理跨会话保持知识。Hippocampus提出高效可扩展的智能体AI内存模块，提供结构化记忆管理支持长期信息存储和高效检索，使智能体在复杂多步任务中保持更好的连贯性和适应性。
...
不同工作负载对LLM精度和延迟有不同要求。MorphServe通过运行时量化层交换和KV缓存调整实现工作负载感知的LLM服务，根据实时负载动态调整量化精度和缓存大小，实现自适应的性能和质量平衡。
...
KV缓存低位量化可能显著降低模型质量。Kitty提出准确高效的2位KV缓存量化方法，采用动态通道级精度提升技术，在运行时识别重要通道并分配更高精度，大幅降低量化误差同时实现与全精度缓存相当的模型质量。
...
基于ML的系统面临不确定性输入时可能做出错误决策。本文探讨ML驱动系统的不确定性处理——当ML模型不再确定时如何保持可靠性？提出拥抱不确定性的框架，通过量化置信度、维护备选方案和设计安全回退机制构建更鲁棒的系统。
...
推测解码中草稿模型的计算开销往往抵消加速效益。PRISM通过参数化重构优化草稿模型推理，识别可在推测过程中共享或简化的参数并进行重构，减少草稿模型计算开销，显著提升推测解码整体效率。
...
扩散模型作为推测解码的草稿生成器具有并行生成优势。SpecDiff-2通过扩展扩散草稿模型对齐训练来加速推测解码，改进对齐训练使扩散草稿更好地匹配目标模型分布，提高草稿token接受率实现更高效推测解码。
...
推测解码被广泛宣传为LLM推理加速的有效方法，但实际部署收益可能远低于理论预期。本文深入分析推测解码在各种模型、硬件和工作负载下的实际性能，通过大量实验揭示真实部署中的性能瓶颈，包括草稿质量、验证开销和批处理效应，为实践者提供基于实证的指导。
...
vLLM的冷启动延迟——从接收请求到开始生成token的时间——可能长达数十秒。本文通过系统剖析识别关键瓶颈，包括模型权重加载、CUDA内核编译和KV缓存初始化，提出一系列优化措施减少生产环境首次服务的响应时间。
...
移动端LLM推理能耗效率受限于DVFS调速器缺乏协调。本文评估最先进移动LLM框架的能耗效率，揭示默认调速器独立决策导致显著延迟或能耗增加。提出CORE——统一协调CPU、GPU和内存频率的能耗感知调速器，实现显著的延迟和能耗改进。
...
异构GPU集群中不同GPU计算能力差异给分布式LLM训练带来挑战。Zorse在异构集群上优化LLM训练效率，智能将不同计算任务分配到最适合的GPU类型，结合动态负载均衡和通信优化实现显著更高成本效益。
...
长上下文LLM推理注意力计算开销随序列长度二次增长。Kascade是实用的稀疏注意力方法，通过分阶段过滤机制逐步减少需计算的token对数量，先快速排除明显不相关token，再对剩余候选项进行更精确注意力计算。
...
学习型优化器十年进展显著，但VeLO等最新进展因依赖JAX和缺乏用户友好包仍难广泛使用。我们引入PyLO——基于PyTorch的库，通过熟悉的torch.optim.Optimizer接口将学习型优化器带给约70%的ML社区。系统贡献包括CUDA加速实现，训练吞吐量从39和50提高到206和191样本/秒。此外发现学习型优化器能显著受益于学习率调度和权重衰减等传统工具的配合。
...
工业界大量信息处理任务以大批量甚至离线方式执行，吞吐量为关键指标。我们提出BatchLLM——通过全局识别公共前缀并共同调度共享前缀请求来优化大批量LLM推理。重新排序请求优先调度解码比例大的请求以更好混合解码token与预填充块，应用以内存为中心的token批处理提高GPU利用率。相比vLLM和SGLang实现1.3-10.8倍性能提升。
...
稀疏卷积驱动自动驾驶和AR/VR中广泛使用的3D点云网络。我们识别体素坐标三个关键属性：整数值、空间有界、几何连续，设计首个体素属性感知GPU引擎Spira。提出高性能一次性搜索算法、有效打包原生处理方案、灵活双数据流执行和网络级并行策略。端到端推理平均1.68倍加速、最高3.04倍。
...
文本到图像生成服务需在高吞吐量和低延迟间平衡。HADIS是混合自适应扩散模型服务系统，结合多种扩散模型优势，根据用户请求复杂度动态选择最适合的模型和参数配置，保持图像质量同时显著降低服务延迟和计算成本。
...
LLM推理部署配置优化涉及众多参数搜索空间。本文研究LLM推理部署配置自动优化问题，通过系统化参数搜索和性能建模，自动为不同模型和硬件组合找到最优并行策略、批大小和量化设置，减少手动调优时间和成本。
...
对话式RAG系统需要跨多轮保持上下文记忆。本文提出了本体引导的长期智能体记忆方法，利用领域本体来组织和索引智能体的记忆，使智能体能够高效检索和利用历史交互中的相关信息，提升了多轮对话的连贯性和准确性。
...
大推理模型的长推理链产生了大量KV缓存。SkipKV通过选择性跳过KV生成和存储来实现高效推理，识别推理链中不重要的步骤并跳过其KV缓存生成，显著减少了长推理链模型的计算和内存开销。
...
推测解码的加速效果取决于草稿token的接受率。本文通过学习最优的随机种子来引导草稿token生成，提高了推测解码中草稿模型的接受率，在不影响输出质量的情况下实现了更快的LLM推理。
...
KV缓存驱逐策略的选择对长上下文推理质量有显著影响。NeSyKV将神经符号架构知识用于LLM推理中的KV缓存驱逐。结合神经网络的模式识别和符号推理的结构化知识，更智能地决定哪些KV缓存条目需要保留。
...
MoE训练和推理中的通信开销限制了系统的可扩展性。本文提出了面向MoE加速的捷径连接专家并行方法，通过引入跨层捷径连接减少专家的通信需求，在保持模型质量的同时降低了MoE并行训练和推理中的通信开销。
...
本文提出了消费级GPU上的局部性感知波束调度方法，用于高效的测试时计算。通过利用波束搜索中候选序列的局部性来减少内存访问，在单个消费级GPU上实现了高效的并行推理。
...
FSDP因内存效率和可扩展性被广泛用于大规模分布式LLM训练，但面临通信开销和内存碎片问题。veScale-FSDP引入了灵活且高性能的FSDP变体，通过优化的分片策略、智能参数预取和异步通信来克服这些限制，在保持PyTorch兼容性的同时实现了显著的吞吐量提升。
...
LLM分布式训练和推理依赖高效跨节点通信。fabric-lib是专门为LLM系统设计的RDMA点对点通信库，通过直接内存访问绕过OS内核和CPU参与，提供低延迟高带宽通信原语，相比传统TCP/IP显著降低跨节点通信开销。
...
超长上下文训练中注意力计算二次增长导致严重的计算和内存瓶颈。MTraining提出面向超长上下文训练的分布式动态稀疏注意力系统，通过动态识别和聚焦关键注意力模式，自适应调整稀疏策略，在保持模型质量的同时大幅降低计算和内存开销。
...
大规模训练作业的可靠性受GPU故障严重影响。本文分析通过备用策略最小化大规模训练可靠性影响的方法，通过预分配备用GPU并在检测到故障时快速切换，结合智能检查点和状态恢复机制，显著降低因硬件故障导致的失败概率。
...
AXLearn是促进大规模深度学习模型可扩展高性能训练的生产系统，独特专注模块化和硬件无关训练。内部接口遵循严格封装，允许组装不同组件促进快速模型开发。AXLearn在扩展时保持常数复杂度而最先进系统为线性或二次复杂度，使得将RoPE等功能集成到数百个模块只需10行代码。同时在Apple开发运营中保持了与最先进系统相当的性能。
...
非IID数据的联邦学习通常使客户端性能降至本地训练基线以下。我们首先对FL中逐层泛化动态进行系统分析，揭示可泛化和任务特定层之间的早期过渡。引入PLayer-FL，在单个训练周期后高效计算新颖联邦敏感性度量来选择最佳分割点。实验表明在广泛任务中持续实现有竞争力性能，同时更公平分配收益并减少客户端性能退化。
...
ML ASIC开发需大量手动优化的底层算子。本文提出面向ML ASIC的智能体算子生成方法，利用LLM智能体自动从硬件规格描述生成和优化专用AI加速器的底层算子代码，通过迭代测试和反馈循环持续改进代码质量，大幅缩短开发周期。
...
AI加速器内核优化耗时且需专业知识。AccelOpt提出自改进的LLM智能体系统，自动生成、测试和优化加速器内核代码，通过分析性能数据和编译器反馈持续迭代改进，在多种加速器平台上生成的代码性能接近或超过手工优化版本。
...
PyTorch模型推理优化通常需深入框架知识。本文探索利用基于LLM的多智能体系统自动优化PyTorch推理，多个专门化智能体协同分析模型计算图、识别优化机会并生成优化代码，自动化推理性能调优过程。
...
AccelOpt是自改进的LLM智能体系统（另一版本），通过迭代优化自动生成和调试AI加速器内核代码，分析性能数据和编译器反馈持续迭代改进内核性能。
...
OSWorld-Human是评估计算机使用智能体效率的基准测试工具，模拟真实用户操作模式来衡量智能体完成桌面任务时的时间和资源消耗，为智能体系统的实用性提供更贴近实际的评估标准。
...
智能体现已广泛应用于软件开发，但构建生产就绪软件工程智能体任务复杂。我们推出OpenHands软件智能体SDK——灵活接口仅需几行代码即可实现智能体并轻松扩展到全功能。相比OpenAI、Claude和Google现有SDK，OpenHands独特集成原生沙箱执行、生命周期控制、模型无关多LLM路由和内置安全分析。生产部署数据表明V1大幅减少系统归因故障。
...
AMD GPU在ML工作负载中展现强大计算能力，但现有内核库多为NVIDIA GPU优化。HipKittens是一套专门为AMD GPU设计的高性能内核库，充分利用AMD GPU的矩阵核心和高速缓存层次，通过针对性的内存访问模式优化、指令调度和并行化策略实现了与NVIDIA GPU上主流内核库相当甚至更优的性能。
...
多GPU AI内核开发涉及复杂的分片、同步和通信管理。ParallelKittens提出系统化简化多GPU AI内核的方法，通过自动化分片和通信管理将复杂的多GPU内核开发简化为直观的编程模式，显著降低开发门槛同时保持高性能。
...
长序列推理中注意力计算开销随序列长度二次增长。BLASST提出基于softmax阈值的动态分块注意力稀疏方法，在计算过程中动态识别和跳过注意力分数低于阈值的块，在不影响质量的前提下大幅减少注意力计算量。
...
注意力机制变体日益增多但PyTorch编译器支持有限。Flashlight是PyTorch编译器针对注意力变体的扩展套件，通过自动识别注意力模式并高效映射到硬件实现跨模型架构的统一加速，同时保持与PyTorch生态的完全兼容。
...
视觉生成模型中的稀疏注意力面临序列维度负载不均衡和通信开销问题。db-SP通过双平衡序列并行加速稀疏注意力，平衡序列维度的计算负载和通信开销，使稀疏注意力在视觉生成工作负载上高效执行。
...
TPU编译器选项调优对效率影响显著但手动搜索耗时。CATWILD是针对TPU工作负载的编译器自动调优工具，在生产环境中自动搜索最优编译选项组合，无需人工干预即可找到高效编译配置。
...
3D高斯溅射展现卓越场景重建质量但速度有限。SwiftGS通过算法和系统协同优化实现GPU上快速3D高斯溅射，结合高效并行渲染算法和GPU友好数据结构，保持渲染质量同时大幅提升训练和推理速度。
...
基于归因的稀疏激活通过分析每个token对输出的贡献度来选择性地激活大语言模型中的神经元。该方法识别并只激活对当前输入最重要的神经元路径，跳过贡献度低的计算，在不进行任何微调的情况下实现了高效的推理加速，同时保持了模型的完整表达能力。
...
Transformer在边缘设备部署受延迟和能耗限制。虽然INT8量化加速了主要矩阵乘法，但softmax路径成为主导瓶颈可能占据注意力总延迟的65%。我们提出IntAttention——首个完全整数注意力流水线，核心是IndexSoftmax——在整数域内替代浮点指数的硬件友好算子。Armv8 CPU上实现高达3.7倍加速和61%能耗降低。
...
尽管低位QAT已有大量工作，仍存在与原生训练的精度差距。我们引入CAGE——通过曲率感知校正增强STE梯度的新QAT方法，源自QAT的多目标视角在损失最小化与量化约束之间取得平衡。引入量化优化帕累托最优解概念并证明光滑非凸环境中强大收敛保证。在类似计算成本下将QAT微调压缩精度损失减半，3位预训练精度匹配先前最佳方法的4位训练。
...
块浮点是量化有效格式但缩放因子选择对精度影响显著。本文提出搜索最优块浮点缩放因子的方法，通过自动搜索每层甚至每数据块的浮点数缩放参数，在保持模型精度同时最大限度压缩数值表示。
...
现代ML模型要求越来越高的算力，促使硬件供应商在其GPU中添加专门的矩阵核心。虽然这些单元解锁了高吞吐量，但它们强加了复杂的编程模型和寻址方案，手动管理非常困难。本文介绍Wave，一个嵌入Python的DSL用于内核编写，自动完成这些复杂的地址计算，让开发者专注于核心计算逻辑。在实验中，它匹配或超越了最先进的内核DSL和库的性能。
...
大规模机器学习加速器需要高效的片上网络来支持集合通信操作。本文提出了一种面向大规模ML加速器的轻量级高吞吐量集合通信NoC，通过优化的路由策略和专用的集合通信硬件支持，在保持低功耗和小面积的同时，为AllReduce等关键通信模式提供了接近线速的吞吐量。
...
面向长上下文的自动语音识别需要高效处理长时间音频输入。本文提出了一个专为长上下文ASR设计的高效系统，通过创新的注意力机制和流式处理架构来解决超长音频转录中的计算和存储瓶颈，在处理长达数小时的音频时仍能保持低延迟和高准确率。
...
数据中心VM的NUMA放置对性能影响显著。本文研究大规模VM NUMA放置问题，提出基于学习的方法自动优化NUMA拓扑分配，通过学习历史放置决策和性能数据预测最优NUMA配置，在遵循约束的同时最大化性能并有效处理尾部延迟。
...
UMAP是广泛使用的降维可视化算法，但GPU版本受限于显存。本文提出在GPU上实现大规模外存UMAP，通过将部分数据存储在CPU内存并智能调度数据移动来突破显存限制，支持处理远超GPU显存容量的数据集。
...
优化器超参数自动调优通常依赖通用方法。本文提出自动调优优化器的算法自动设计方法，将优化器设计空间参数化并使用自动搜索算法探索，能自动发现针对特定工作负载和硬件的最优配置，减少人工调参依赖。
...
大规模TPU集群利用效率对ML生产力有直接影响。本文介绍ML集群效率优化方法，通过ML生产力良率指标改进TPU系统的大规模运行效率，综合考量硬件利用率、任务完成率和资源浪费等维度。
...
现代ML技术栈涉及多层软硬件，性能分析需覆盖各阶段。XProf是开放、可扩展的现代ML栈分析系统，提供统一的数据采集和分析接口，支持自定义插件，覆盖框架层、运行时层和硬件层，为ML系统性能调优提供全面可观测性。
...
将PyTorch模型部署到边缘设备需专门工具链。ExecuTorch是统一PyTorch设备端部署解决方案，提供从模型导出、优化到部署的完整工作流，支持移动设备、嵌入式和微控制器等多种硬件平台。
...
联邦学习中LLM缺乏对模型更新如何影响特定客户端的细粒度归因。ProToken提出token级别归因方法，精确量化联邦LLM中每个token对各个客户端的贡献，不增加通信开销实现细粒度模型行为理解和公平性评估。
...
对抗性多臂老虎机问题是RL经典问题，传统算法在大规模场景下运行时间过高。本文提出具有次线性运行时间的实用算法，通过新颖算法设计和数据结构优化，保持对抗性场景理论保证的同时实现远超传统方法的计算效率。
...
Linux页面缓存驱逐策略传统上使用LRU等启发式方法。LearnedCache将基于感知器的驱逐策略集成到eBPF框架中，通过轻量级ML模型预测缓存行未来访问模式，比传统LRU更准确识别重要缓存内容，提高系统I/O性能。
...
多轮LLM对话中模型观点和立场可能随时间漂移。SAT-Eval是评估多轮对话中偏好漂移的框架，测量模型在长对话中偏好和立场的一致性，为对话系统质量保证提供新评估维度。
...
许多复合AI系统本质近似——ML组件是概率模型，非ML组件是启发式的。我们提出approxMLIR——可重用精度感知编译工具链，引入approx MLIR方言作为定义近似的统一集中接口，以及approx-opt基于MLIR的可重用优化器。评估显示approxMLIR可有效表示许多常见近似选择并持续实现更高加速。
...
本文提出面向业务问题的自动化特征工程方法，通过加速迭代周期更快解决实际问题。系统自动发现、生成和选择最有价值的特征组合，减少对数据科学家手动调参的依赖。
...
本文提出学习引导的生命周期影响分析设计优化方法，利用ML模型预测产品生命周期各阶段环境影响，在设计阶段就能评估和优化产品可持续性指标，为绿色设计提供数据驱动决策支持。
...
基于嵌入的向量搜索支撑推荐和RAG等重要应用，但向量索引需存储高维嵌入和大量元数据，总大小可达原始数据数倍。我们提出LEANN——存储高效索引，动态重新计算嵌入而非存储它们，并压缩最先进邻近图索引同时保持搜索精度，将索引大小减少高达50倍同时保持SOTA精度和可比较延迟。
...
模拟存内计算硬件具有高能效但面临非理想特性挑战。本文提出评估神经网络在模拟存内计算硬件上可部署性的框架，通过建模模拟计算的噪声和变化等非理想特性，评估模型实际推理精度和鲁棒性。
...
去中心化学习中网络拓扑和协作策略对训练效果有重要影响。SONAR是一个基准测试工具，用于评估去中心化学习中的拓扑和协作效果，系统地测量不同网络拓扑结构和协作策略对收敛速度和最终质量的影响。
...
差分隐私与分布式训练存在张力。本文提出在零冗余优化器中集成差分隐私的分布式学习方法，通过精心设计的噪声注入和梯度裁剪策略，在保护数据隐私的同时保持训练效率，实现隐私保护和模型性能的有效平衡。
...
GNN在隐私敏感领域广泛应用但私有推理计算开销巨大。G-HEMP是利用同态加密进行大规模GCN多GPU私有推理的快速框架，通过优化加密域中矩阵乘法和通信模式，在数据完全加密条件下实现高效GNN推理。
...
自回归模型连贯性好但生成慢，扩散模型速度快但连贯性差。TiDAR提出混合生成架构——在扩散中思考、在自回归中表达，结合扩散模型并行生成优势和自回归模型的语言连贯性，显著减少生成步数同时保持文本质量。
...
生成模型正在重塑直播行业。我们提出StreamDiffusionV2——无需训练的交互式直播视频扩散流水线，集成SLO感知批处理调度器、块调度器、sink-token引导滚动KV缓存和运动感知噪声控制器。引入可扩展流水线编排跨去噪步骤和网络层并行化。四块H100 GPU上14B模型0.5秒内渲染首帧达58.28 FPS，1.3B模型达64.52 FPS。
...
生产级LLM部署缺乏系统方法来评估基础设施变化对输出一致性的风险。基础设施漂移——硬件配置、软件版本和数据分布的渐进变化——会在不明显降低标准指标的情况下导致模型行为发生意外变化。我们提出DriftBench，一个用于测量和预测LLM服务系统中基础设施漂移的基准测试框架，提供标准化的漂移场景、检测指标和预测模型，使运维人员能够在漂移影响生产服务质量之前主动识别和缓解漂移。
...
LLM可能表现出迎合用户错误观点的谄媚倾向。PARROT是专门评估LLM谄媚鲁棒性的基准测试，系统测量模型在说服和一致性测试中的表现，为评估和改进LLM推理鲁棒性提供标准化工具，涵盖多种对话场景和难度级别。
...
RLVR方法通常依赖大规模数据和计算资源。本文系统衡量RLVR在低数据和低计算条件下的有效性，通过受控实验揭示该方法在资源受限场景下的性能边界，为难以获取大规模资源的研究者提供实用指导。
